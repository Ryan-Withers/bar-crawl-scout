...
describe('scoring engine (reconciliation vs league scoring_settings)', () => {
  ...
  it('scores a QB line (6pt pass TD, -1 INT)', () => {
    // 305*0.04 + 3*6 - 1 + 22*0.1 = 12.2 + 18 - 1 + 2.2 = 31.4
    expect(scoreStats({ pass_yd: 305, pass_td: 3, pass_int: 1, rush_yd: 22 }, scoring)).toBe(31.4);
  });
  it('ignores stat keys the league does not score', () => {
    expect(scoreStats({ rec: 5, def_int: 2, blk_kick: 1 }, scoring)).toBe(2.5);
  });
  it('reconciles a full RB/receiving line within rounding', () => {
    ...
    const expected = 6 * 0.5 + 74 * 0.1 + 41 * 0.1 + 6 - 2; // 18.5 (rush_att is not scored)
