...
// One rostered Sleeper player {n,p,t,s} -> enriched {name,pos,team,proj,bye,starter}.
export function enrichPlayer(pl, ks, md) {
...
export function rosterFor(rostersData, handle, ks, md) {
  const r = rostersData && rostersData.byHandle ? rostersData.byHandle[handle] : null;
  return r ? r.players.map((pl) => enrichPlayer(pl, ks, md)) : null;
}

// Your enriched roster (Ryan = you).
export function myRoster(rostersData, ks, md) {
  return rosterFor(rostersData, RYAN, ks, md);
}
