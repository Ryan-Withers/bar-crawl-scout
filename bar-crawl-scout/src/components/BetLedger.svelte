...
<section class="ledger">
  ...
  <p class="blurb">Everyone starts even on pretend dollars; the sharpest read (and the luckiest) climbs. <a href="/book" use:link>Back to the markets →</a></p>
...
<style>
  ...
  .blurb { font-family: 'IBM Plex Mono', monospace; font-size: 12px; color: var(--muted); margin: 0 0 16px; line-height: 1.6; max-width: 78ch; } .blurb a { color: #0C8C4A; }
  ...
  /* Slips tile across a wide screen instead of stacking one per row. */
  .mybets { display: grid; grid-template-columns: repeat(auto-fit, minmax(min(100%, 300px), 1fr)); gap: 10px; align-items: start; }
  .bet { min-width: 0; background: var(--barroom-lift); border: 1px solid var(--line); border-radius: 10px; padding: 11px 14px; }
  ...
  @media (max-width: 620px) {
    .commish .orow { grid-template-columns: 1fr auto; }
    .odesc, .owk { grid-column: 1 / -1; white-space: normal; }
    .oact button { width: 44px; height: 44px; } /* thumb-sized on a phone */
  }
